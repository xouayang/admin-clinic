<template>
  <div>
    ...
    <v-card class="container">
      ...
      >
        ບັນທຶກຂໍ້ມູນພື້ນຖານ (ຂໍ້ມູນຄົນເຈັບທີ່ເຄີຍມາປີ່ນປົວ)
      </div>
      <v-row class="d-flex align-center col-12">
        <v-row class="container col-12">
          <v-col md="6" sm="12">
            <v-text-field
              v-model="dataFrom_result.name"
              label="ຊື່"
              outlined
              dense
              color="#9155FD"
          ...
          <v-col md="6" sm="12">
            <v-text-field
              v-model="dataFrom_result.address"
              label="ທີ່ຢູ່"
              outlined
              dense
              color="#9155FD"
...
export default {
  ...
  computed: {
    dataFrom_result() {
      return this.$store.state.result.historyDataId
    },
  },
   mounted() {
    this.firstCheck.name = this.dataFrom_result.name
    this.firstCheck.address = this.dataFrom_result.address
    this.firstCheck.tel = this.dataFrom_result.tel
  },
  methods: {
    validateNumber(e) {
      if (e.key === '-') {
    ...
    back() {
        this.$router.push('/patients/allPatient')
    }
  },
}
</script>
